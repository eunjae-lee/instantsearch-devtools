...
<div class="border border-gray-300 rounded-sm shadow-sm py-4 px-4">
  <div>
    <span class="bg-indigo-100 text-indigo-800 py-1 px-4 rounded-full">SearchBox</span>
  </div>
  <div class="mt-2 ml-2 flex items-center">
    ...
    <span class="ml-1">
      Query: {#if query}{query}{:else}<span class="italic text-gray-500">(empty string)</span>{/if}
    </span>
...
instantSearchInstance.getUiState()['${indexName}'].query
`}</code>
    </pre>
  </div>
  {/if}
